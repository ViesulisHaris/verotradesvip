...
async function testVRatingComponent() {
  console.log('Starting VRating component final test...');
  ...
  try {
    ...
    await page.screenshot({ path: 'vrating-test-initial-load.png', fullPage: true });
    console.log('Screenshot taken: vrating-test-initial-load.png');
    ...
    if (await vratingCard.isVisible()) {
      ...
      console.log('❌ VRating card not found');
    ...
    const expandButton = await page.locator('button:has-text("Performance"), button:has-text("Breakdown"), button[aria-label*="expand"], .expand-button').first();
    ...
    if (await journalingAdherenceSection.isVisible()) {
      ...
      const emotionalDisciplineItem = await page.locator('text=Emotional Discipline').first();
      ...
      // Check if "Emotional Discipline" is not in Journaling Adherence
      // This is tricky because "Emotional Discipline" is also a category name
      // We need to check if it appears as an item within Journaling Adherence
      const journalingSection = journalingAdherenceSection.locator('..');
      const emotionalDisciplineInJournaling = await journalingSection.locator('text=Emotional Discipline').count();
      
      if (emotionalDisciplineInJournaling > 1) { // More than the category title itself
        console.log('❌ "Emotional Discipline" still present in Journaling Adherence (should be removed)');
      } else {
        console.log('✅ "Emotional Discipline" correctly removed from Journaling Adherence');
      }
    } else {
    ...
    if (await vratingScore.isVisible()) {
      ...
      console.log('❌ Total VRating score not found');
    }
    ...
    fs.writeFileSync('vrating-final-test-report.json', JSON.stringify(testReport, null, 2));
    console.log('Test report saved to vrating-final-test-report.json');
