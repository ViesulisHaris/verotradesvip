...
async function authenticate(page) {
  ...
  const tests = [
    {
      ...
      test: async () => {
        ...
        return loginTitle && loginTitle.includes('VeroTrade');
      }
    },
    {
    ...
    {
      name: 'Login successful - redirected to dashboard',
      test: async () => {
  ...
  for (const test of tests) {
    try {
      ...
      testResults.authentication[test.name] = result;
      ...
      if (result) {
        ...
        log(`✗ ${test.name}`, 'error');
        await takeScreenshot(page, `auth-failed-${test.name.toLowerCase().replace(/\s+/g, '-')}`);
      }
    } catch (error) {
      testResults.authentication[test.name] = false;
      testResults.summary.totalTests++;
      testResults.summary.failedTests++;
  ...
  // Navigate to calendar page after authentication
...
async function testEdgeCases(page) {
  ...
  const tests = [
    {
      name: 'Handles months with no trades',
      test: async () => {
        // Navigate to a month far in the future/past
        await page.click('button:has-text(">")');
        await page.click('button:has-text(">")');
        await page.click('button:has-text(">")');
        await page.waitForTimeout(2000);
        ...
        const gridVisible = await waitForElement(page, '.grid.grid-cols-7', 2000);
        const noTradeDays = await page.$$('[class*="border-green-500"], [class*="border-red-500"]');
        
        return gridVisible && noTradeDays.length === 0;
      }
    },
    {
      name: 'Handles empty data gracefully',
      test: async () => {
        const loadingIndicator = await page.$$('text=Loading trading data');
        const errorMessage = await page.$$('text=Error');
        return loadingIndicator.length === 0 && errorMessage.length === 0;
      }
    }
  ];
  ...
  for (const test of tests) {
    try {
      const result = await test.test();
      testResults.edgeCases[test.name] = result;
      testResults.summary.totalTests++;
      if (result) {
        testResults.summary.passedTests++;
        log(`✓ ${test.name}`);
        ...
        testResults.summary.failedTests++;
        log(`✗ ${test.name}`, 'error');
        await takeScreenshot(page, `edge-case-failed-${test.name.toLowerCase().replace(/\s+/g, '-')}`);
      }
    } catch (error) {
      testResults.edgeCases[test.name] = false;
      testResults.summary.totalTests++;
      testResults.summary.failedTests++;
      log(`✗ ${test.name} - Error: ${error.message}`, 'error');
    }
...
async function runTests() {
  log('Starting comprehensive calendar page testing with authentication...');
  ...
  try {
    ...
    fs.writeFileSync('./CALENDAR_TEST_REPORT_WITH_AUTH.md', report);
...
function generateReport() {
  ...
  let report = `# Calendar Page Comprehensive Testing Report (With Authentication)\n\n`;
  ...
  Object.entries(testResults.authentication).forEach(([test, result]) => {
    report += `- ${result ? '✅' : '❌'} ${test}\n`;
  });
  report += `\n`;
