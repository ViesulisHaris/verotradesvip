...
async function createTestStrategyWithTrades() {
  ...
  try {
    ...
    // Create some test trades for this strategy
    const trades = [];
    const startDate = new Date('2024-01-01');
    
    for (let i = 0; i < 10; i++) {
      const tradeDate = new Date(startDate);
      tradeDate.setDate(startDate.getDate() + i * 7); // Weekly trades
      
      const pnl = Math.random() > 0.5 ? 
        Math.round(Math.random() * 1000 + 100) : // Random profit between $100-$1100
        -Math.round(Math.random() * 500 + 50);   // Random loss between $50-$550
    ...
    const stats = {
      ...
      avg_hold_period: 1440 // 24 hours in minutes
    };
...
createTestStrategyWithTrades().catch(console.error);