...
async function improvedTradeLoggingTest() {
  console.log('🚀 Starting improved trade logging test...');
  ...
  try {
    ...
    try {
      // Symbol field (not pair)
      await page.waitForSelector('input[name="symbol"], input[placeholder*="AAPL"], input[placeholder*="symbol"]', { timeout: 5000 });
      await page.fill('input[name="symbol"], input[placeholder*="AAPL"], input[placeholder*="symbol"]', 'EUR/USD');
      ...
      // Entry price
      await page.waitForSelector('input[name="entry_price"]', { timeout: 5000 });
      await page.fill('input[name="entry_price"]', '1.0850');
      console.log('✅ Entry price filled');
      
      // Exit price
      await page.waitForSelector('input[name="exit_price"]', { timeout: 5000 });
    ...
    console.log('\n✅ Improved trade logging test completed!');
    ...
    console.error('❌ Improved test failed:', error);
    return {
      success: false,
      error: error.message,
      consoleMessages: consoleMessages,
...
improvedTradeLoggingTest().then(result => {
  if (result.success) {
    ...
    console.log('\n❌ FAILURE: Issues found during improved testing.');
    if (result.error) {
