...
function saveResults() {
  const filename = `trade-generation-test-results-final-${Date.now()}.json`;
...
async function runTradeGenerationTest() {
  ...
  try {
    log('Starting trade generation browser test (FINAL)...');
    ...
    const authData = await page.evaluate(() => {
      ...
      if (authData) {
        try {
          ...
          return {
            hasToken: !!parsed.currentSession?.access_token,
    ...
    // Step 4: Make API request to generate test data with proper authentication
    testResults.testPhase = 'api-request';
    log('Making API request to generate test trades...');
    
    // Use the extracted token to make authenticated API request
    const apiResponse = await page.evaluate(async (config, authInfo) => {
      try {
    ...
    // Step 13: Test error handling scenarios
    testResults.testPhase = 'error-handling-verification';
    await testErrorHandling(page, authData);
...
async function testErrorHandling(page, authData) {
  log('Testing error handling scenarios...');
  
  // Test with invalid action
  try {
    const invalidResponse = await page.evaluate(async (authInfo) => {
      try {
        const response = await fetch('/api/generate-test-data', {
          method: 'POST',
          headers: {
  ...
  try {
    const missingActionResponse = await page.evaluate(async (authInfo) => {
      try {
        const response = await fetch('/api/generate-test-data', {
          method: 'POST',
          headers: {
