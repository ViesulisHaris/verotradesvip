...
function saveResults() {
  const filename = `trade-generation-test-results-fixed-${Date.now()}.json`;
...
async function runTradeGenerationTest() {
  ...
  try {
    log('Starting trade generation browser test (FIXED)...');
    ...
    // Step 3: Make API request to generate test data
    testResults.testPhase = 'api-request';
    log('Making API request to generate test trades...');
    
    // First, let's check if we have authentication cookies
    const cookies = await context.cookies();
    log(`Found ${cookies.length} cookies after login`);
    
    // Use the browser's context to make the API request with proper authentication
      ...
      try {
        ...
        const response = await fetch('/api/generate-test-data', {
          ...
          headers: {
            ...
            // Let the browser automatically include cookies
          },
    ...
    if (apiResponse.status !== 200) {
      throw new Error(`API request failed with status ${apiResponse.status}: ${JSON.stringify(apiResponse.data)}`);
    }
    
    if (!apiResponse.ok) {
      throw new Error(`API response not ok: ${JSON.stringify(apiResponse.data)}`);
    }
    
    log('API request successful');
    testResults.tradeData = apiResponse.data;
    
    // Step 5: Verify trade count and basic structure
    testResults.testPhase = 'trade-count-verification';
    const trades = apiResponse.data.trades || [];
    ...
    if (actualTradeCount !== TEST_CONFIG.expectedResults.totalTrades) {
      throw new Error(`Expected ${TEST_CONFIG.expectedResults.totalTrades} trades, got ${actualTradeCount}`);
    }
    
    // Step 6: Verify win rate
...
async function testErrorHandling(page) {
  ...
  try {
    ...
    log(`Invalid action response status: ${invalidResponse.status}`);
    
    if (invalidResponse.status === 400 || invalidResponse.status === 500) {
      log('Error handling for invalid action working correctly');
    } else {
      log('Warning: Error handling for invalid action may not be working as expected', 'warning');
    }
  } catch (error) {
    log(`Error testing invalid action: ${error.message}`, 'error');
  }
  
  // Test with missing action
  try {
    const missingActionResponse = await page.evaluate(async () => {
      try {
        const response = await fetch('/api/generate-test-data', {
          method: 'POST',
          headers: {
    ...
    log(`Missing action response status: ${missingActionResponse.status}`);
    
    if (missingActionResponse.status === 400 || missingActionResponse.status === 500) {
      log('Error handling for missing action working correctly');
    } else {
      log('Warning: Error handling for missing action may not be working as expected', 'warning');
    }
  } catch (error) {
    log(`Error testing missing action: ${error.message}`, 'error');
  }
}
