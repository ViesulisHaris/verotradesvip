...
// SAFETY CONFIRMATION
console.log('🔒 SAFE SERVICE ROLE DATA GENERATION');
console.log('===================================');
console.log('This script uses service role key to bypass authentication');
...
async function getOrCreateTestUser() {
  ...
  // If no users exist, we need to create one via auth first
  console.log('⚠️ No existing users found. Creating test user...');
  
  // Create a test user with auth
  const testEmail = 'test-emotional-data@example.com';
  ...
  return { id: authData.user.id, email: testEmail };
}
...
async function safeServiceRoleDataGeneration() {
  try {
    console.log('🚀 STARTING SAFE SERVICE ROLE DATA GENERATION');
    console.log('==============================================\n');
    ...
    if (verificationSuccess) {
      console.log('\n🎉 SAFE SERVICE ROLE DATA GENERATION COMPLETED SUCCESSFULLY!');
      console.log('========================================================');
    ...
    console.error('❌ Error during safe service role data generation:', error.message);
  }
}

// Run the safe data generation
safeServiceRoleDataGeneration().then(() => {
  console.log('\n✅ Safe service role data generation process completed');
}).catch(error => {
