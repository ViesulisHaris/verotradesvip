...
async function testDashboardMenuButtons() {
  ...
  if (loginPage) {
    ...
    await page.screenshot({ path: 'after-login-dashboard-screenshot.png', fullPage: true });
    console.log('After-login dashboard screenshot saved to after-login-dashboard-screenshot.png');
    
    // Navigate to dashboard again if needed
  ...
  const mobileSidebarSelectors = [
    ...
    '[class*="mobile"][class*="sidebar"]',
    '[class*="sidebar"][class*="mobile"]'
  ];
  ...
  const desktopSidebarSelectors = [
    ...
    '[class*="desktop"][class*="sidebar"]',
    '[class*="sidebar"][class*="desktop"]',
    '.sidebar'
  ];
  ...
  for (const href of navLinkSelectors) {
    const elements = await page.$$(href);
    if (elements.length > 0) {
      navLinksFound[href] = true;
      console.log(`✓ Found ${elements.length} navigation link(s): ${href}`);
      
      // Get text of first link
      const text = await elements[0].textContent();
      console.log(`  First link text: "${text}"`);
    } else {
  ...
  await page.screenshot({ path: 'desktop-view-screenshot.png', fullPage: true });
  console.log('Desktop view screenshot saved to desktop-view-screenshot.png');
  ...
  const fs = require('fs');
  fs.writeFileSync('dashboard-menu-test-results.json', JSON.stringify({
    testResults,
    consoleLogs,
    timestamp: new Date().toISOString()
  }, null, 2));
  
  console.log('\nTest results saved to dashboard-menu-test-results.json');
...
// Run the test
testDashboardMenuButtons().catch(console.error);