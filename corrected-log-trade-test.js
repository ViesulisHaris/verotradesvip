...
class CorrectedLogTradeTester {
  ...
  async init() {
    console.log('🚀 Initializing corrected log trade page test...');
  ...
  async testInteractiveElements() {
    ...
    for (const marketType of marketTypes) {
      try {
        // Use text content selector instead of :has-text
        const selector = `button[type="button"]`;
        const buttons = await this.page.$$(selector);
        
        // Find button with correct text content
        let targetButton = null;
        for (const button of buttons) {
          const text = await this.page.evaluate(el => el.textContent, button);
          if (text.toLowerCase().includes(marketType.toLowerCase())) {
            targetButton = button;
            break;
          }
        }
        ...
        if (!targetButton) {
          throw new Error(`Button for ${marketType} not found`);
        }
        
        // Check initial state - look for white background or any non-gold state
        const initialState = await this.page.evaluate(el => {
          return !Array.from(el.classList).some(cls => cls.includes('bg-verotrade-gold-primary'));
        }, targetButton);
        
        // Click to toggle
        await targetButton.click();
        await this.sleep(500); // Increased wait time
        
        // Check state after click - look for any gold background class
        const afterState = await this.page.evaluate(el => {
          return Array.from(el.classList).some(cls => cls.includes('bg-verotrade-gold-primary'));
        }, targetButton);
    ...
    for (const side of sides) {
      try {
        ...
        if (!targetButton) {
          throw new Error(`Button for ${side} not found`);
        }
        ...
        const correctColor = side === 'Buy' ? 
          await this.page.evaluate(el => el.classList.contains('bg-profit/20'), targetButton) :
          await this.page.evaluate(el => el.classList.contains('bg-loss/20'), targetButton);
    ...
    try {
      // Find strategy button by checking all buttons and finding one with "None" or "Select Strategy" text
      const strategyButton = await this.page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button[type="button"]'));
        return buttons.find(btn =>
          btn.textContent?.includes('None') ||
          btn.textContent?.includes('Select Strategy')
        );
      });
      ...
      if (strategyButton) {
        await this.page.evaluate(() => {
          const buttons = Array.from(document.querySelectorAll('button[type="button"]'));
          const strategyBtn = buttons.find(btn =>
            btn.textContent?.includes('None') ||
            btn.textContent?.includes('Select Strategy')
          );
    ...
    try {
      // Find emotion button by checking all buttons and finding one with "Neutral" text
      const emotionButton = await this.page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button[type="button"]'));
        return buttons.find(btn => btn.textContent?.includes('Neutral'));
      });
    ...
    try {
      ...
      if (saveButton) {
        ...
        const hasBeamAnimation = await this.page.evaluate(() => {
          ...
          const hasBorderBeamClass = button.classList.contains('border-beam');
          const hasRelativePosition = computedStyle.position === 'relative';
          return hasBorderBeamClass && hasRelativePosition;
        });
        ...
        else this.testResults.summary.warnings.push('Border beam animation may not be properly implemented');
        this.testResults.summary.totalTests++;
      }
    } catch (error) {
      this.testResults.animations.border_beam = {
  ...
  async testFormFunctionality() {
    ...
    const formInputs = [
      { selector: 'input[placeholder*="AAPL"], input[placeholder*="BTCUSD"]', value: 'AAPL', name: 'symbol' },
      ...
      { selector: 'input[placeholder="0.00"]', value: '150.00', name: 'stop_loss', index: 2 },
      { selector: 'input[placeholder="0.00"]', value: '200.00', name: 'take_profit', index: 3 },
      { selector: 'input[placeholder="0.00"]', value: '500.00', name: 'pnl', index: 4 }
    ];

    for (const input of formInputs) {
      try {
        let targetSelector = input.selector;
        
        // For inputs with same placeholder, use index to target specific one
        if (input.index !== undefined) {
          const allInputs = await this.page.$$(input.selector);
          if (allInputs.length > input.index) {
            targetSelector = allInputs[input.index];
          }
        } else {
        ...
        await this.page.type(targetSelector, input.value);
        
        const valueEntered = await this.page.evaluate(el => el.value, targetSelector);
    ...
    try {
      ...
      await this.page.type('input[placeholder*="AAPL"], input[placeholder*="BTCUSD"]', 'AAPL');
      await this.page.type('input[placeholder*="0.00"]', '100');
      await this.page.type('input[type="date"]', '2024-01-15');
      
      // Select market type
      const marketButtons = await this.page.$$('button[type="button"]');
  ...
  async testIntegration() {
    ...
    try {
      const authState = await this.page.evaluate(() => {
        ...
        // Find logout button by checking all buttons
        const buttons = Array.from(document.querySelectorAll('button'));
        const hasLogoutButton = buttons.find(btn =>
    ...
    try {
      ...
      await this.page.type('input[placeholder*="AAPL"], input[placeholder*="BTCUSD"]', testData.symbol);
  ...
  async generateReport() {
    ...
    const report = {
      ...
      testSuite: 'Log Trade Page Corrected Test',
      version: '1.0.0',
      environment: {
...
if (require.main === module) {
  const tester = new CorrectedLogTradeTester();
...
module.exports = CorrectedLogTradeTester;