/**
 * Comprehensive TorchCard Implementation Test
 * 
 * This script tests the TorchCard component across all containers on the trades page
...
// Expected containers with TorchCard on the trades page
const EXPECTED_TORCHCARD_CONTAINERS = [
  {
    name: 'Statistics Card 1 - Total Trades',
    selector: 'div.grid > div:nth-child(1)',
    description: 'First statistics card showing total trades'
  },
  {
    name: 'Statistics Card 2 - Total P&L',
    selector: 'div.grid > div:nth-child(2)',
    description: 'Second statistics card showing total P&L'
  },
  {
    name: 'Statistics Card 3 - Win Rate',
    selector: 'div.grid > div:nth-child(3)',
    description: 'Third statistics card showing win rate'
  },
  {
    name: 'Statistics Card 4 - Top Emotion',
    selector: 'div.grid > div:nth-child(4)',
    description: 'Fourth statistics card showing top emotion'
  },
  {
    name: 'Filter Bar',
    selector: 'div.space-y-4 > div:nth-child(1)',
    description: 'Filter bar container'
  },
  {
    name: 'Sort Controls',
    selector: 'div.space-y-4 > div:nth-child(2)',
    description: 'Sort controls container'
  },
  {
    name: 'Table Controls (Page Size & Pagination)',
    selector: 'div.scroll-item > div:nth-child(1)',
    description: 'Table controls with page size selector and pagination'
  },
  {
    name: 'Table Headers',
    selector: 'div.scroll-item > div:nth-child(2)',
    description: 'Table headers container'
  },
  {
    name: 'Individual Trade Cards',
    selector: 'div.space-y-3.mt-4 > div',
    description: 'Individual trade cards (multiple elements)'
  },
  {
    name: 'No Trades Message',
    selector: 'div.space-y-3.mt-4 > div.text-center',
    description: 'No trades message container (when no trades exist)'
  }
];
...
async function testTorchCardImplementation(page) {
  ...
  const torchCardExists = await page.evaluate(() => {
    ...
    const elements = document.querySelectorAll('[class*="relative overflow-hidden rounded-xl border border-white/5"]');
  ...
  // Check TorchCard CSS classes
  const hasCorrectClasses = await page.evaluate(() => {
    const elements = document.querySelectorAll('[class*="relative overflow-hidden rounded-xl border border-white/5"]');
    if (elements.length === 0) return false;
    
    // Check first element for correct TorchCard classes
    const firstElement = elements[0];
    return firstElement.classList.contains('relative') && 
           firstElement.classList.contains('overflow-hidden') && 
           firstElement.classList.contains('rounded-xl');
  });
  
  logTestResult(
    'TorchCard has correct CSS classes',
    hasCorrectClasses,
    'Elements have relative, overflow-hidden, and rounded-xl classes'
  );
}

// Test 2: Verify all expected containers have TorchCard
async function testTorchCardContainers(page) {
  console.log('\n=== Test 2: TorchCard Container Verification ===');
  
  for (const container of EXPECTED_TORCHCARD_CONTAINERS) {
    try {
      // Check if container exists
      const containerExists = await page.locator(container.selector).first().isVisible({ timeout: 5000 });
      
      if (!containerExists) {
        // Some containers might not exist (like no trades message when there are trades)
        if (container.name.includes('No Trades')) {
          logTestResult(
            `${container.name} - Not Applicable`,
            true,
            'Container not needed when trades exist'
          );
          continue;
        }
        
        logTestResult(
          `${container.name} - Container Missing`,
          false,
          `Container not found with selector: ${container.selector}`
        );
        continue;
      }
      
      // Check if container has TorchCard styling
      const hasTorchCard = await page.locator(container.selector).first().evaluate(el => {
        return el.classList.contains('relative') && 
               el.classList.contains('overflow-hidden') && 
               el.classList.contains('rounded-xl') &&
               el.style.border && el.style.border.includes('white/5');
      });
      
      logTestResult(
        `${container.name} - Has TorchCard`,
        hasTorchCard,
      ...
      logTestResult(
        ...
        false,
        `Error checking container: ${error.message}`
      );
    }
  }
}

// Test 3: Test visual appearance of torch effect
async function testTorchEffectVisual(page) {
  console.log('\n=== Test 3: Torch Effect Visual Appearance ===');
  ...
  const colorCorrect = await page.evaluate(() => {
    ...
    return {
      innerGlowBg,
      borderBeamBg,
      innerGlowWhite: innerGlowBg.includes('255, 255, 255'), // White RGB
      borderBeamGold: borderBeamBg.includes('197, 160, 101') // Gold RGB
    };
  });
...
// Test 4: Test mouse tracking functionality
async function testMouseTracking(page) {
  console.log('\n=== Test 4: Mouse Tracking Functionality ===');
  ...
  // Get bounding box of the TorchCard
  ...
  for (const position of testPositions) {
    ...
    const trackingWorks = await page.evaluate((pos) => {
      ...
      return innerGlowBg.includes(`${relativeX}px ${relativeY}px`) &&
             borderBeamBg.includes(`${relativeX}px ${relativeY}px`);
...
// Test 6: Test torch effect only appears on hover
...
async function runTests() {
  console.log('🔥 Starting TorchCard Comprehensive Test\n');
  ...
  try {
    ...
    await testTorchCardContainers(page);
...
function generateTestReport() {
  ...
  const reportPath = path.join(TEST_CONFIG.screenshotDir, 'torch-card-test-report.json');
