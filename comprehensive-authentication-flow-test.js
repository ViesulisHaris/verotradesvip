...
async function testLoginPageAccess(page) {
  ...
  try {
    ...
    const loginForm = await waitForElement(page, 'form[data-testid="login-form"], form');
    ...
    const hasErrors = await checkElementExists(page, '[data-testid="error"], .error, .alert-error');
    
    if (loginForm && emailInput && passwordInput && submitButton && !hasErrors) {
...
async function testAuthenticationProcess(page) {
  ...
  try {
    // Fill in login form
    await page.type('input[type="email"], input[name="email"]', TEST_CONFIG.testUser.email);
    await page.type('input[type="password"], input[name="password"]', TEST_CONFIG.testUser.password);
