...
async function testImmediateAttentionManual() {
  console.log('🔍 Starting Manual Immediate Attention UI Behavior Tests...\n');
  ...
  try {
    // Create a simple test HTML page with VRatingCard
...
<head>
    ...
    <style>
        ...
        .group:hover .group-hover\\:shadow-3xl { box-shadow: 0 35px 60px -12px rgba(0, 0, 0, 0.25); }
    </style>
</head>
<body>
    <div id="root"></div>
    ...
    <script type="text/babel">
        ...
        const { TrendingUp, Shield, Target, Brain, BookOpen, ChevronDown, ChevronUp, Trophy, Star, Award, Zap, Info } = lucide;
        ...
        const scenarios = {
            elite: {
                overallScore: 9.2,
                categories: {
                    profitability: { name: 'Profitability', score: 9.5, weight: 30, contribution: 2.85, keyMetrics: ['Win Rate: 85%', 'P&L: +12.5%'], icon: TrendingUp },
                    riskManagement: { name: 'Risk Management', score: 9.0, weight: 25, contribution: 2.25, keyMetrics: ['Max DD: 3.2%'], icon: Shield },
                    consistency: { name: 'Consistency', score: 9.1, weight: 20, contribution: 1.82, keyMetrics: ['P&L StdDev: 4.5%'], icon: Target },
                    emotionalDiscipline: { name: 'Emotional Discipline', score: 9.3, weight: 15, contribution: 1.395, keyMetrics: ['Positive Emotions: 92%'], icon: Brain },
                    journalingAdherence: { name: 'Journaling Adherence', score: 8.8, weight: 10, contribution: 0.88, keyMetrics: ['Completeness: 95%'], icon: BookOpen }
                },
                adjustments: [],
            ...
            poor: {
                overallScore: 4.0,
                categories: {
                    profitability: { name: 'Profitability', score: 4.2, weight: 30, contribution: 1.26, keyMetrics: ['Win Rate: 42%'], icon: TrendingUp },
                    riskManagement: { name: 'Risk Management', score: 3.5, weight: 25, contribution: 0.875, keyMetrics: ['Max DD: 22.5%'], icon: Shield },
                    consistency: { name: 'Consistency', score: 4.8, weight: 20, contribution: 0.96, keyMetrics: ['P&L StdDev: 28%'], icon: Target },
                    emotionalDiscipline: { name: 'Emotional Discipline', score: 3.0, weight: 15, contribution: 0.45, keyMetrics: ['Positive Emotions: 35%'], icon: Brain },
                    journalingAdherence: { name: 'Journaling Adherence', score: 5.0, weight: 10, contribution: 0.5, keyMetrics: ['Completeness: 65%'], icon: BookOpen }
                },
                adjustments: [],
                calculatedAt: new Date().toISOString()
            },
            mixed: {
                overallScore: 6.0,
                categories: {
                    profitability: { name: 'Profitability', score: 7.5, weight: 30, contribution: 2.25, keyMetrics: ['Win Rate: 65%'], icon: TrendingUp },
                    riskManagement: { name: 'Risk Management', score: 4.2, weight: 25, contribution: 1.05, keyMetrics: ['Max DD: 15.8%'], icon: Shield },
                    consistency: { name: 'Consistency', score: 6.8, weight: 20, contribution: 1.36, keyMetrics: ['P&L StdDev: 18%'], icon: Target },
                    emotionalDiscipline: { name: 'Emotional Discipline', score: 5.5, weight: 15, contribution: 0.825, keyMetrics: ['Positive Emotions: 58%'], icon: Brain },
                    journalingAdherence: { name: 'Journaling Adherence', score: 8.0, weight: 10, contribution: 0.8, keyMetrics: ['Completeness: 92%'], icon: BookOpen }
                },
                adjustments: [],
                calculatedAt: new Date().toISOString()
            }
        ...
        function VRatingCard({ vRatingData }) {
            ...
            const categoriesWithIcons = [
                { ...vRatingData.categories.profitability, key: 'profitability', icon: TrendingUp },
                { ...vRatingData.categories.riskManagement, key: 'riskManagement', icon: Shield },
                { ...vRatingData.categories.consistency, key: 'consistency', icon: Target },
                { ...vRatingData.categories.emotionalDiscipline, key: 'emotionalDiscipline', icon: Brain },
                { ...vRatingData.categories.journalingAdherence, key: 'journalingAdherence', icon: BookOpen }
            ];
            
            return React.createElement('div', { className: 'card-solid-enhanced rounded-xl p-6 relative overflow-hidden' }, [
                                            ...
                                            poorCategories.map(category => {
                                                ...
                                                return React.createElement('div', { key: category.key, className: 'flex items-center justify-between text-xs' }, [
                                                    React.createElement('div', { className: 'flex items-center gap-2' }, [
                                                        React.createElement(CategoryIcon, { className: \`w-3 h-3 \${performanceLevel.color}\` }),
        ...
        // Main App component
        function App() {
            const [currentScenario, setCurrentScenario] = useState('elite');
        ...
        window.runTests = async () => {
            ...
            const isCollapsedInitially = !document.querySelector('.mt-4.space-y-3') || 
    ...
    await page.screenshot({ 
      path: 'immediate-attention-manual-test-final-state.png',
      fullPage: true 
    });
    console.log('\n📸 Screenshot saved: immediate-attention-manual-test-final-state.png');
