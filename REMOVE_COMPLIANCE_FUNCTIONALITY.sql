...
DO $$
BEGIN
    -- Drop policies for compliance table if they existed
    IF EXISTS (
        SELECT 1
        FROM pg_policies
        WHERE tablename IN ('strategy_rule_compliance', 'compliance_table', 'rule_compliance')
    ) THEN
        ...
        DROP POLICY IF EXISTS "Users can view own compliance" ON compliance_table;
        DROP POLICY IF EXISTS "Users can insert own compliance" ON compliance_table;
        DROP POLICY IF EXISTS "Users can update own compliance" ON compliance_table;
        DROP POLICY IF EXISTS "Users can delete own compliance" ON compliance_table;
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Compliance tables still exist';
    ELSE
        RAISE NOTICE 'All compliance tables successfully removed';
    END IF;
END $$;
