...
(async () => {
  console.log('🔍 Starting TradesPage Infinite Render Test v5 - With Proper Authentication...');
  ...
  try {
    ...
    if (tradesLink) {
      console.log('🔗 Found trades link, clicking...');
      await tradesLink.click();
      
      // Wait for navigation
      console.log('⏱️ Waiting for trades page to load...');
    ...
    try {
      await page.waitForSelector('h1:has-text("Trade History")', { timeout: 10000 });
      ...
      if (tradeElements.length > 0) {
        ...
        // Let's continue with test anyway to see what happens
      }
    }
