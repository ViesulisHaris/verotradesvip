...
console.log('🚀 300 TRADES GENERATOR FOR STRATEGY PERFORMANCE MODAL');
console.log('======================================================');
console.log('Generating exactly 300 diverse trades for testing...');
...
const SPECS = {
  ...
  SYMBOLS: {
    'Stock': ['AAPL', 'GOOGL', 'MSFT', 'TSLA', 'AMZN', 'META', 'NVDA', 'AMD', 'NFLX'],
    'Crypto': ['BTCUSD', 'ETHUSD'],
    'Forex': ['EURUSD', 'GBPUSD', 'USDJPY', 'AUDUSD'],
    'Futures': ['ES', 'NQ', 'YM', 'RTY', 'GC', 'CL', 'NG']
  },
  PNL_RANGES: {
    win: { min: 25, max: 750 },
    loss: { min: -500, max: -20 }
...
function generateEmotionalStates() {
  ...
  // 40% chance of having additional emotions (1-2 more)
  const emotions = [primaryEmotion];
  if (Math.random() < 0.4) {
    const additionalCount = getRandomInt(1, 2);
...
function generateRealisticQuantity(market, symbol) {
  switch (market) {
    ...
    case 'Crypto':
      return getRandomFloat(0.1, 5.0, 4);
    case 'Forex':
...
function generateRealisticPrice(market, symbol) {
  switch (market) {
    ...
    case 'Crypto':
      ...
      return getRandomFloat(100, 10000, 2);
    case 'Forex':
      return getRandomFloat(1.0, 1.5, 5);
    case 'Futures':
...
async function generateTrades(strategies) {
  console.log('\n📊 Generating 300 diverse trades for strategy performance modal testing...');
  ...
  console.log(`✅ Generated ${shuffledTrades.length} diverse trades for strategy performance modal testing`);
...
async function insertTrades(trades) {
  ...
  const batchSize = 15; // Slightly larger batch size for efficiency
  let insertedCount = 0;
  let errorCount = 0;
...
async function verifyGeneratedData() {
  ...
  try {
    ...
    // Verify we have exactly 300 trades
    if (trades.length !== 300) {
      console.log(`⚠️  Expected 300 trades, but found ${trades.length}`);
    } else {
      console.log(`✅ Exactly 300 trades found as expected`);
    }
...
async function main() {
  try {
    console.log('🚀 Starting 300 trades generation for strategy performance modal testing...\n');
    ...
    if (insertedCount > 0) {
      ...
      console.log('\n🎉 300 TRADES GENERATION COMPLETED!');
      console.log('📝 Summary:');
      console.log(`  - Generated ${insertedCount} diverse trades`);
      console.log(`  - 68% win rate maintained (${SPECS.WINNING_TRADES} wins, ${SPECS.LOSING_TRADES} losses)`);
      console.log(`  - Even distribution across 5 strategies (60 trades each)`);
      console.log(`  - Balanced market types (40% Stocks, 30% Crypto, 20% Forex, 10% Futures)`);
      console.log(`  - All 10 emotional states represented`);
      console.log(`  - Realistic P&L ranges ($25-$750 wins, -$20 to -$500 losses)`);
      console.log(`  - 90-day date range with weekday-only trading`);
      console.log(`  - Market hours (6 AM - 4 PM)`);
      
      console.log('\n🎯 Next Steps:');
      console.log('1. Navigate to /dashboard to see trade analytics');
      console.log('2. Go to /confluence to view emotional analysis');
      console.log('3. Test the strategy performance modal with comprehensive data');
      console.log('4. Verify all 300 trades are properly associated with strategies');
    ...
    console.error('\n❌ FATAL ERROR during test data generation:', error.message);
