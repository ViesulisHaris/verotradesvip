...
console.log('🔍 COMPREHENSIVE DATABASE DIAGNOSTIC TOOL');
...
// Helper function to test a client connection
async function testClientConnection(client, clientType) {
  if (!client) {
    logTestResult(`${clientType} Client Connection`, false, { error: 'Client not initialized' });
    return false;
  }
  ...
  try {
    ...
    if (error && !error.message.includes('does not exist')) {
      logTestResult(`${clientType} Client Connection`, false, { error: error.message });
      return false;
    } else {
      logTestResult(`${clientType} Client Connection`, true, { message: 'Basic connection successful' });
      return true;
    }
  } catch (error) {
    logTestResult(`${clientType} Client Connection`, false, { error: error.message });
...
async function testBasicTableQueries(client, clientType) {
  if (!client) return;
...
async function testComplexQueries(client, clientType) {
  if (!client) return;

  // Test strategies with rules join
  try {
...
async function testAuthenticationScenarios() {
  ...
  // Test anon client
  const anonConnected = await testClientConnection(anonClient, 'Anon');
  if (anonConnected) {
    await testBasicTableQueries(anonClient, 'Anon');
    await testComplexQueries(anonClient, 'Anon');
  }

  // Test service role client
  const serviceConnected = await testClientConnection(serviceClient, 'Service');
  if (serviceConnected) {
    await testBasicTableQueries(serviceClient, 'Service');
    await testComplexQueries(serviceClient, 'Service');
...
function analyzeResults() {
  ...
  const complianceErrors = allTests.filter(test => test.hasComplianceError);
  const permissionErrors = allTests.filter(test => test.hasPermissionError);
  const cacheErrors = allTests.filter(test => test.hasCacheError);
...
async function runComprehensiveDiagnostic() {
  ...
  try {
    ...
    const fs = require('fs');
    const reportPath = `comprehensive-database-diagnostic-${Date.now()}.json`;
