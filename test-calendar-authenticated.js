...
async function testCalendarWithAuthentication() {
  console.log('🚀 Starting calendar page testing with proper authentication...\n');
  ...
  try {
    ...
    // Check if we're logged in (should be redirected to dashboard or calendar)
    ...
    if (currentUrl.includes('/dashboard') || currentUrl.includes('/calendar')) {
      ...
      const errorElement = await page.locator('text=Invalid').or(page.locator('.text-red-300')).first();
    ...
    console.log('\n📍 Step 4: Navigating to calendar page...');
    ...
    console.log('\n📍 Step 5: Checking if calendar page loads...');
    const calendarTitle = await page.locator('h1').first().textContent();
    if (calendarTitle && (calendarTitle.includes('Calendar') || calendarTitle.includes('2025'))) {
      testResults.calendarPageLoads = true;
      console.log('✅ Calendar page loads successfully');
    } else {
      console.log('❌ Calendar page failed to load, title:', calendarTitle);
    }
    
    console.log('\n📍 Step 6: Checking for AuthSessionMissingError...');
    ...
    console.log('\n📍 Step 7: Verifying authentication state initialization...');
    ...
    const authInitMessages = consoleMessages.filter(msg => 
      ...
      (msg.text.includes('Initializing') || msg.text.includes('Session retrieved') || msg.text.includes('Auth state change'))
    );
    ...
    console.log('\n📍 Step 8: Checking monthly performance metrics positioning...');
    // Wait for calendar to load
    ...
    if (await metricsSection.isVisible()) {
      ...
      if (calendarBox && metricsBox && metricsBox.y > calendarBox.y + calendarBox.height) {
        ...
        console.log(`Calendar box: ${JSON.stringify(calendarBox)}`);
        console.log(`Metrics box: ${JSON.stringify(metricsBox)}`);
      }
    } else {
      console.log('❌ Monthly performance metrics section not found');
    }
    
    console.log('\n📍 Step 9: Testing calendar navigation...');
    ...
    console.log('\n📍 Step 10: Testing trades functionality...');
    ...
    console.log('\n📍 Step 12: Testing responsive layout...');
    ...
    await page.screenshot({ 
      path: 'calendar-test-results-authenticated.png',
      fullPage: true 
    });
...
// Run the test
testCalendarWithAuthentication()
  .then(results => {
    ...
    console.log(`Calendar Page Loads: ${results.calendarPageLoads ? '✅' : '❌'}`);
    ...
    const reportPath = path.join(__dirname, 'calendar-test-report-authenticated.json');
