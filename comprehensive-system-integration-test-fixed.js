...
async function getJWTToken(page) {
  log('AUTH', 'Extracting JWT token from browser storage');
  ...
  try {
    ...
    if (token) {
      ...
      // Try alternative approach - get from cookies
      ...
      if (authCookie) {
        log('AUTH', `✅ JWT token found in cookie: ${authCookie.name}`, 'success');
        return authCookie.value;
      } else {
        log('AUTH', '❌ No JWT token found in storage or cookies', 'error');
        return null;
      }
    }
  } catch (error) {
...
async function runComprehensiveIntegrationTest() {
  console.log('🚀 Starting Comprehensive System Integration Test with Fixed Authentication');
