...
async function testColorCoding(page, scenario) {
  ...
  try {
    ...
    const testResultsText = await page.$eval('.bg-slate-800.rounded-lg.p-4', el => {
      return el.innerText;
    });
    
    results.details.push(`Color test results: ${testResultsText}`);
    ...
    for (const [categoryName, expectedColor] of Object.entries(scenario.expectedCategoryColors)) {
      const categorySelector = `[data-category="${categoryName}"]`;
      try {
        ...
        if (categoryElement) {
          ...
          const expectedCategoryColorClass = COLOR_CLASSES[expectedColor][0];
          
          if (!categoryClasses.includes(expectedCategoryColorClass)) {
            results.passed = false;
            results.errors.push(`Category ${categoryName} color mismatch. Expected ${expectedCategoryColorClass}, found ${categoryClasses}`);
          } else {
            results.details.push(`✅ Category ${categoryName} color correct: ${expectedColor}`);
          }
        }
      } catch (error) {
        // Category might not be visible until expanded
        results.details.push(`ℹ️ Category ${categoryName} not visible (needs expansion)`);
      }
    }
...
async function testCalculationLogic(page, scenario) {
  ...
  try {
    ...
    results.details.push(`Calculation test results: ${testResultsText}`);
    
    // Verify overall score matches expected
    const overallScoreElement = await page.$('.text-4xl.font-bold');
    const actualScore = await page.evaluate(el => el.textContent, overallScoreElement);
    const numericScore = parseFloat(actualScore.split('/')[0]);
    
    if (Math.abs(numericScore - scenario.expectedOverallScore) > 0.1) {
      results.passed = false;
      results.errors.push(`Overall score mismatch. Expected ${scenario.expectedOverallScore}, found ${numericScore}`);
    } else {
      results.details.push(`✅ Overall score correct: ${numericScore}`);
    }
...
async function testUIBehavior(page, scenario) {
  ...
  try {
    ...
    results.details.push(`UI test results: ${testResultsText}`);
...
async function generateTestReport(testResults) {
  ...
  markdownReport += `- **Success Rate:** ${((testResults.summary.passedTests / testResults.summary.totalTests) * 100).toFixed(1)}%\n\n`;
