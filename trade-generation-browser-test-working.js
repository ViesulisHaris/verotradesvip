...
function saveResults() {
  const filename = `trade-generation-test-results-working-${Date.now()}.json`;
...
async function runTradeGenerationTest() {
  ...
  try {
    log('Starting trade generation browser test (WORKING)...');
    ...
    // Use extracted token to make authenticated API request
    const apiResponse = await page.evaluate(async (authInfo) => {
      try {
