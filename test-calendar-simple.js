...
async function testCalendarDirectly() {
  console.log('🚀 Starting direct calendar page testing (bypassing login)...\n');
  ...
  try {
    ...
    console.log('\n📍 Step 2: Mocking authentication and navigating to calendar...');
    // Mock the authentication context to bypass login
    await page.addInitScript(() => {
      ...
      // Mock React useContext for AuthContext
      const originalUseContext = window.React?.useContext;
      if (window.React && originalUseContext) {
    ...
    console.log('\n📍 Step 4: Checking if calendar page loads...');
    const calendarTitle = await page.locator('h1').first().textContent();
    if (calendarTitle && calendarTitle.includes('Calendar')) {
      testResults.calendarPageLoads = true;
      console.log('✅ Calendar page loads successfully');
    } else {
      console.log('❌ Calendar page failed to load');
    }
    
    console.log('\n📍 Step 5: Checking for AuthSessionMissingError...');
    ...
    console.log('\n📍 Step 6: Verifying authentication state initialization...');
    ...
    if (authInitMessages.length > 0 || consoleMessages.some(msg => msg.text.includes('useAuth'))) {
    ...
    console.log('\n📍 Step 7: Checking monthly performance metrics positioning...');
    ...
    console.log('\n📍 Step 8: Testing calendar navigation...');
    ...
    console.log('\n📍 Step 9: Testing trades functionality...');
    ...
    console.log('\n📍 Step 11: Testing responsive layout...');
    ...
    await page.screenshot({ 
      path: 'calendar-test-results-simple.png',
      fullPage: true 
    });
...
testCalendarDirectly()
  .then(results => {
    ...
    const reportPath = path.join(__dirname, 'calendar-test-report-simple.json');
