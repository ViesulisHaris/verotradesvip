...
function saveResults() {
  const filename = `data-verification-test-results-simple-${Date.now()}.json`;
...
async function runDataVerificationTest() {
  ...
  try {
    log('Starting data verification test (SIMPLE)...');
    ...
    // Step 3: Extract auth token and test "verify-data" API endpoint
    testResults.testPhase = 'basic-endpoint-test';
    log('Testing "verify-data" API endpoint with explicit auth token...');
    
    // Extract the auth token first
    ...
    if (!authData) {
      throw new Error('Could not extract auth token from localStorage');
    }
    
    log(`Auth token extracted successfully (length: ${authData.length})`);
    
    // Use page.evaluate to make the API call with explicit auth token
    const verificationResponse = await page.evaluate((token) => {
    ...
    // Step 4: Test statistical accuracy (if we have verification data)
...
async function testErrorHandling(page) {
  ...
  // Extract the auth token first (same as in main test)
  const authData = await page.evaluate(() => {
    const authKey = 'sb-auth-token';
    const authData = localStorage.getItem(authKey);
    if (authData) {
