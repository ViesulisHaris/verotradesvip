...
function executeQuery(query, description, callback) {
...
executeQuery(tableQuery, 'strategy_rule_compliance table', function(tableResult) {
  ...
  executeQuery(columnQuery, 'compliance_percentage column', function(columnResult) {
    ...
    let functionChecksCompleted = 0;
    
    for (let i = 0; i < complianceFunctions.length; i++) {
      const funcName = complianceFunctions[i];
      const funcQuery = `
        SELECT COUNT(*) as count 
      ...
      executeQuery(funcQuery, `${funcName} function`, function(funcResult) {
        ...
        functionChecksCompleted++;
        
        // When all function checks are done, continue with triggers
        if (functionChecksCompleted === complianceFunctions.length) {
          ...
          let triggerChecksCompleted = 0;
          
          for (let j = 0; j < complianceTriggers.length; j++) {
            const triggerName = complianceTriggers[j];
            const triggerQuery = `
              SELECT COUNT(*) as count 
            ...
            executeQuery(triggerQuery, `${triggerName} trigger`, function(triggerResult) {
              ...
              triggerChecksCompleted++;
              
              // When all trigger checks are done, continue with database functionality tests
              if (triggerChecksCompleted === complianceTriggers.length) {
                ...
                executeQuery(strategiesQuery, 'strategies table', function(strategiesResult) {
                  if (strategiesResult) {
                  ...
                  executeQuery(tradesQuery, 'trades table', function(tradesResult) {
                    if (tradesResult) {
                    ...
                    executeQuery(rulesQuery, 'strategy_rules table', function(rulesResult) {
                      if (rulesResult) {
                      ...
                      executeQuery(tradesStructureQuery, 'trades table structure', function(tradesStructureResult) {
