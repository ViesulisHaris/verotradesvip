...
async function testDashboardAccessibility(page) {
  ...
  try {
    ...
    const dashboardTitle = await utils.waitForElement(page, 'h1');
...
async function testTradingStatistics(page) {
  ...
  try {
    ...
    const totalPnL = await utils.getElementText(page, '*[text*="Total P&L"], *:contains("Total P&L")');
    const winRate = await utils.getElementText(page, '*[text*="Win Rate"], *:contains("Win Rate")');
    const profitFactor = await utils.getElementText(page, '*[text*="Profit Factor"], *:contains("Profit Factor")');
    const totalTrades = await utils.getElementText(page, '*[text*="Total Trades"], *:contains("Total Trades")');
    
    utils.logTestResult(
      'Key Metrics Display',
      !!(totalPnL || winRate || profitFactor || totalTrades),
      `P&L: ${!!totalPnL}, Win Rate: ${!!winRate}, Profit Factor: ${!!profitFactor}, Total Trades: ${!!totalTrades}`
    );
    
    await utils.takeScreenshot(page, 'trading-stats', 'metrics-display');
    
    return metricsCards.length > 0 || hasMetricsGrid;
  } catch (error) {
...
async function testEmotionRadar(page) {
  ...
  try {
    ...
    const hasEmotionalSection = await utils.checkElementExists(page, '*:contains("Emotional Analysis")');
...
async function testPnLChart(page) {
  ...
  try {
    ...
    const hasPnLSection = await utils.checkElementExists(page, '*:contains("P&L Performance")');
...
async function testRecentTradesTable(page) {
  ...
  try {
    ...
    const hasRecentTradesSection = await utils.checkElementExists(page, '*:contains("Recent Trades")');
...
async function testDataFetching(page) {
  ...
  try {
    ...
    const hasLoadingIndicator = await utils.checkElementExists(page, '*:contains("Loading")');
...
async function testErrorHandlingAndLoading(page) {
  ...
  try {
    ...
    const hasLoadingText = await utils.checkElementExists(page, '*:contains("Loading")');
    ...
    const hasRetryButton = await utils.checkElementExists(page, '*:contains("Try Again"), button[onclick*="reload"]');
...
async function runDashboardTests() {
  ...
  try {
    ...
    const reportPath = path.join(__dirname, 'DASHBOARD_COMPREHENSIVE_TEST_REPORT.json');
    fs.writeFileSync(reportPath, JSON.stringify(reportData, null, 2));
    console.log(`\n📄 Detailed report saved to: ${reportPath}`);
    
    // Generate markdown report
    const markdownReport = generateMarkdownReport(reportData);
    const markdownPath = path.join(__dirname, 'DASHBOARD_COMPREHENSIVE_TEST_REPORT.md');
...
function generateMarkdownReport(reportData) {
  ...
  return `# Dashboard Comprehensive Test Report
