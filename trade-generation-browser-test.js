...
function saveResults() {
  const filename = `trade-generation-test-results-${Date.now()}.json`;
...
async function runTradeGenerationTest() {
  ...
  try {
    log('Starting trade generation browser test...');
    ...
    // Use the browser's context to make the API request with cookies
...
async function verifyTradeDataIntegrity(trades) {
  ...
  const emotions = [...new Set(trades.map(trade => trade.emotional_state))];
