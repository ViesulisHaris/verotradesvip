...
async function robustTradeLoggingTest() {
  console.log('🚀 Starting robust trade logging test...');
  ...
  try {
    ...
    try {
      ...
      if (strategyOptions.length > 1) {
        ...
        // Select a strategy (not first empty option)
    ...
    // Test 5: Fill out trade form with exact selectors from TradeForm
    ...
    try {
      ...
      // Symbol field - use placeholder as fallback
      try {
        await page.waitForSelector('input[name="symbol"]', { timeout: 3000 });
        await page.fill('input[name="symbol"]', 'EUR/USD');
        console.log('✅ Symbol field filled (name selector)');
      } catch (error) {
        await page.fill('input[placeholder*="AAPL"], input[placeholder*="symbol"]', 'EUR/USD');
        console.log('✅ Symbol field filled (placeholder selector)');
      }
      ...
      // Quantity
      try {
        await page.waitForSelector('input[name="quantity"]', { timeout: 3000 });
        await page.fill('input[name="quantity"]', '1000');
        console.log('✅ Quantity filled (name selector)');
      } catch (error) {
        await page.fill('input[placeholder*="0.00"]', '1000');
        console.log('✅ Quantity filled (placeholder selector)');
      }
      
      // Entry price - use multiple selectors
      let entryPriceFilled = false;
      ...
      for (const selector of entrySelectors) {
        try {
          ...
          await page.fill(selector, '1.0850');
          console.log(`✅ Entry price filled (${selector})`);
      ...
      if (!entryPriceFilled) {
        console.log('❌ Could not fill entry price field');
      }
      
      // Exit price - use multiple selectors
      let exitPriceFilled = false;
      const exitSelectors = [
      ...
      for (const selector of exitSelectors) {
        try {
          await page.waitForSelector(selector, { timeout: 2000 });
          await page.fill(selector, '1.0900');
          console.log(`✅ Exit price filled (${selector})`);
          exitPriceFilled = true;
          break;
        } catch (error) {
          continue;
        }
      ...
      if (!exitPriceFilled) {
        console.log('❌ Could not fill exit price field');
      }
      
      // P&L
      try {
        await page.waitForSelector('input[name="pnl"]', { timeout: 3000 });
        await page.fill('input[name="pnl"]', '5.00');
        console.log('✅ P&L filled (name selector)');
      } catch (error) {
        await page.fill('input[type="number"]:nth-of-type(3)', '5.00');
        console.log('✅ P&L filled (nth selector)');
      }
      
      // Date
      const today = new Date().toISOString().split('T')[0];
      try {
        await page.waitForSelector('input[name="date"], input[type="date"]', { timeout: 3000 });
      ...
      // Entry time
      try {
        await page.waitForSelector('input[name="entry_time"], input[type="time"]', { timeout: 3000 });
        await page.fill('input[name="entry_time"], input[type="time"]', '09:30');
        console.log('✅ Entry time filled');
      } catch (error) {
        console.log('❌ Could not fill entry time field');
      }
      
      // Exit time
      try {
        await page.waitForSelector('input[name="exit_time"]', { timeout: 3000 });
        await page.fill('input[name="exit_time"]', '10:15');
        console.log('✅ Exit time filled');
      } catch (error) {
        console.log('❌ Could not fill exit time field');
      }
      
      console.log('✅ Trade form fill attempt completed');
    } catch (error) {
    ...
    console.log('\n✅ Robust trade logging test completed!');
...
robustTradeLoggingTest().then(result => {
  if (result.success) {
    ...
    console.log('\n❌ FAILURE: Issues found during robust testing.');
    if (result.error) {
